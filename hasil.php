...
if(isset($_GET['kode'])){
    ...
    $sql = "delete
            FROM
             process_log where id='$_GET[kode]'";
    $query=$db_object->db_query($sql);
    echo "<h2 class='text-danger'>Data Telah Terhapus</h2> <br>";
    echo "<meta http-equiv=refresh content=2;URL='index.php?menu=hasil'>";
    // $jumlah=$db_object->db_num_rows($query);
}
...
//                        $maka = implode(" Dan ", $kom2);
                            ...
                            echo "<td>".$view."</td>";
                            echo "<td>";
                            echo "<a href='export/CLP.php?id_process=".$row['id']."&start=".format_date2($row['start_date'])."&end=".format_date2($row['end_date'])."' "
                                    . "class='btn btn-app btn-light btn-xs btn' target='blank'>
                                    <i class='ace-icon fa fa-sm fa-print'></i>
                                    
                                </a>";
                            echo "</td>";
                            echo "<td><form method='post'><center><a href='?menu=hasil&kode=".$row['id']."' class='btn btn-danger btn-xs '>Hapus</a></center></form></td>";
